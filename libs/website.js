...
module.exports = function(logger){
    ...
    var minerpage = function(req, res, next){
        logger.debug(logSystem, 'minerpagecalled');

        portalStats.getAllWorkers(function()
        {
        });
    ...
    var workerpage = function(req, res, next){
        logger.debug(logSystem, 'workerpagecalled');
    ...
    var payouts = function(req, res, next){
        var address = req.params.address || null;
        var coin = req.params.coin || null;

        if (address !== null && coin!== null){
            portalStats.getPayoutsByCoinAddress(coin, address, function(data){
                res.write(data.coin.toString + ' '+ data.address.toString() +' '+ data.payout.toString());
                res.end();
            });
        }
        else
