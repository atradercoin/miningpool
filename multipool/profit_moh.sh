#!/bin/bash
# Search/replace MOH with ticker symbol of your desired payout coin. To
# enumerate for additional coins, add a line for EVERY place you see MOH. Leave things 'BTC' alone. :)
...
echo $dayslength
MOHCoinPrice=$(redis-cli hget Exchange_Rates MOHcoin)
TotalEarned=0
TotalEarnedMOHCoin=0
redis-cli hset Pool_Stats CurDaysLength "$dayslength"
redis-cli del Pool_Stats:CurrentShift:WorkerBtc
redis-cli del Pool_Stats:CurrentShift:WorkerMOHCoin
redis-cli del Pool_Stats:CurrentShift:Algos
redis-cli del Pool_Stats:CurrentShift:AlgosMOHCoin
redis-cli del Pool_Stats:CurrentShift:Coins
redis-cli del Pool_Stats:CurrentShift:CoinsMOHCoin
...
do
        ...
        logkey2MOHCoin="Pool_Stats:CurrentShift:AlgosMOHCoin"
        ...
        do
                ...
                logkeyMOHCoin="Pool_Stats:CurrentShift:CoinsMOHCoin"
                ...
                then
                        ...
                        do
                                ...
                                MOHCoinEarned=$(echo "scale=8;$thisEarned / $MOHCoinPrice" | bc -l)
                                coinTotalMOHCoin=$(echo "scale=8;$coinTotalMOHCoin + $MOHCoinEarned" | bc -l)
                                AlgoTotalMOHCoin=$(echo "scale=8;$AlgoTotalMOHCoin + $MOHCoinEarned" | bc -l)
                               echo "$WorkerName earned $MOHCoinEarned from $CoinName"
redis-cli hincrbyfloat Pool_Stats:CurrentShift:WorkerMOHCoin "Total" "$MOHCoinEarned"
redis-cli hincrbyfloat Pool_Stats:CurrentShift:AlgosMOHCoin "Total" "$MOHCoinEarned"
redis-cli hincrbyfloat Pool_Stats:CurrentShift:Algos "Total" "$thisEarned"
redis-cli hincrbyfloat Pool_Stats:CurrentShift:WorkerMOHCoin "$WorkerName" "$MOHCoinEarned"
...
redis-cli hincrbyfloat Pool_Stats:CurrentShift:WorkerBtc "$WorkerName" "$thisEarned"
                        ...
                        redis-cli hset "$logkeyMOHCoin" "$CoinName" "$coinTotalMOHCoin"
                        #echo "$CoinName: $coinTotal"
fi
        done< <(redis-cli hkeys Coin_Names_"$line")
        redis-cli hset "$logkey2" "$line" "$AlgoTotal"
        redis-cli hset "$logkey2MOHCoin" "$line" "$AlgoTotalMOHCoin"
TotalEarned=$(echo "scale=8;$TotalEarned + $AlgoTotal" | bc -l)
TotalEarnedMOHCoin=$(echo "scale=8;$TotalEarnedMOHCoin + $AlgoTotalMOHCoin" | bc -l)
